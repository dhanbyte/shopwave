      ...
      <div className="container flex items-center gap-4 py-3">
        ...
        <div className="hidden md:flex items-center gap-4">
            {mainCategories.filter(c => c !== currentCategory).map(c => (
                <Link key={c} href={`/category/${c.toLowerCase()}`} className="text-sm font-medium text-gray-600 hover:text-brand transition-colors">{c === 'Home' ? 'Home & Kitchen' : c === 'Tech' ? 'Tech Accessories' : c}</Link>
            ))}
        </div>
        ...
        <nav className="ml-auto flex items-center gap-1 sm:gap-3">
          ...
          {user ? (
            ...
            <>
              ...
              <Link href="/account" className="md:hidden relative rounded-full p-2 hover:bg-gray-100 transition-colors" aria-label="Login">
